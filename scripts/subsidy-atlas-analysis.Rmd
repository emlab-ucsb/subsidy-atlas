...
>>>>>>> 25ffb54437b937d4f6a5b8be812b0f3d457b0918
...
=======
  ggtitle(paste0(top_flag_states[i], ' Distant Water Fishing Fleet', sep = ''))+
...
# Save map pdf
ggsave(plot_map, filename = paste0(pdf_flag_dir, top_flag_states[i], "_flag_state_map.pdf"), width = 10, units = "in")
...
if(nrow(flag_subsidy_dat) > 0){
  ...
  subsidy_plot <- ggplot(flag_subsidy_dat, aes(x = category, y = value/1e6, fill = type))+
    ...
    ggtitle(paste0(top_flag_states[i], ' Estimated Fisheries Subsidies (2018 $USD, millions', sep = ''))+
...
=======
  ggsave(subsidy_plot, filename = paste0(pdf_flag_dir, top_flag_states[i], "_subsidy_plot.pdf"), width = 10, units = "in")
...
eez_map <- ggplot()+
  ...
  ggtitle(paste0("Distant Water Fishing in the ", top_eez_names_long[i]))+ #fontface='bold', colour = "white", size = 12)+
  #ggtitle(paste0(top_eezs[i], ' EEZ Fishing Effot by Flag State', sep = ''))+
  maptheme
...
#plot_map pdfs
ggsave(eez_map, filename = paste0(pdf_eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_connectivity_map.pdf"), width = 10, units = "in")
...
### Make Plots 
  ...
  ggsave(filename = paste0(pdf_eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_heat_map.pdf"), eez_combined_plot, width = 10, units = "in")
...
pdf(file.path = pdf_flag_dir, paste0("plot", i, ".pdf", sep = ""))
...
staple_pdf(input_files = c("CHN_flag_state_map.pdf", "CHN_subsidy_plot.pdf"), output_filepath = file.path(pdf_flag_dir, 'Full_pdf.pdf'))

##


output_file <- file.path(pdf_flag_dir, paste('Full_CHN.pdf', sep = ""))
staple_pdf(input_directory = pdf_flag_dir, output_filepath = output_file)

###
setwd("pdf_flag_dir") ##
infiles <- Sys.glob("CHN_*.pdf") ##
outfile <- "CHN.pdf" ##
...
p.1 = "CHN_flag_state_map.pdf"
p.2 = "CHN_subsidy_plot.pdf"

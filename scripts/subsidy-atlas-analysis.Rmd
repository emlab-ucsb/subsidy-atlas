...
country_dir <- here::here("results", "country_tables/")
...
## For loop to run top 10 countries
 for(i in 1:length(top_flag_states)){

# comment this out when loop runs - South Korea as a test map
#i <- 4
...
# Write country data
write_csv(flag_dat, path = paste0(country_dir, "flag_state_dat_", top_flag_states[i], ".csv"))
...
#plot_map
ggsave(plot_map, filename = paste0(map_dir, "flag_state_map_", top_flag_states[i], ".png"), width = 10, units = "in")
...
#plot_map
ggsave(eez_map, filename = paste0(map_dir, "EEZ_map_", top_eezs[i], ".png"), width = 10, units = "in")

}
```

Now we need to pull raw, aggregated GFW data for each EEZ on our list. 

...
pdf_dir <- here::here("results", "pdf_results/")
if (dir.exists(pdf_dir) == F) {
...
maptheme <- theme_minimal()+
  theme(plot.background = element_rect(fill = "grey27"),
...
plot_map <- ggplot()+
  ...
  #ggtitle(paste0(top_flag_states[i]))+
  #labs(caption = "Countriesdksfjs;aldkfja;dlskf")+
  labs(caption = "Distant water fishing fleet of [i]")+
  #ggtitle(paste0(top_flag_states[i]))+
  maptheme

# Save map png
ggsave(plot_map, filename = paste0(country_dir, top_flag_states[i], "_flag_state_map.png"), width = 10, units = "in")

# Save map pdf
ggsave(plot_map, filename = paste0(pdf_dir, top_flag_states[i], "_flag_state_map.pdf"), width = 10, units = "in")
...
if(nrow(flag_subsidy_dat) > 0){
  ...
  ggsave(subsidy_plot, filename = paste0(pdf_dir, top_flag_states[i], "_subsidy_plot.pdf"), width = 10, units = "in")
...
```{r Galapagos test}
flag_dat_JPN2 <- flag_state_mapping_wlines %>%
  dplyr::filter(flag == "JPN") 

# Write flag state connectivity data
write_csv(flag_dat_JPN2, path = paste0(country_dir, top_flag_states[i], "_flag_state_dat.csv"))

plot_map_JPN2 <- ggplot()+
  geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.1)+ # world EEZs (transparent, light grey border lines)
  geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.1)+ # world countries (dark grey, white border lines)
  geom_sf(data = country_map %>% dplyr::filter(iso3 == "JPN"), fill = "darkmagenta", color = NA, size = 0.1) + # highlighted flag state (magenta, white border lines)
  geom_sf(data = EEZ_map %>% dplyr::filter(iso_tr1 == top_flag_states[i]), fill = NA, alpha = 0.5, color = NA, size = 0.1) +
  geom_sf(data = EEZ_map %>% dplyr::filter(mrgid %in% flag_dat_JPN2$eez_code == "8426"), fill = "slateblue", alpha = 0.5, color = NA, size = 0.1) + # highlighted EEZs (slate blue)
  geom_sf(data = flag_dat_JPN2 %>% dplyr::filter(eez_code == "8426"), col = "darkgoldenrod", size = 0.25) + 
  #labs(caption = "Countriesdksfjs;aldkfja;dlskf")+
  labs(capion = paste0(top_flag_states[i]))+
  #ggtitle(paste0(top_flag_states[i]))+
  maptheme
...
#plot_map pdfs
ggsave(eez_map, filename = paste0(eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_connectivity_map.pdf"), width = 10, units = "in")
...
ggsave(eez_map, filename = paste0(pdf_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_connectivity_map.pdf"), width = 10, units = "in")
...
for(i in 1:length(top_eezs)){
  ...
  pdf(file=paste0(filenames[i], ".pdf"))
  
}

for (i in 1:length(top_flag_states)) {
  
pdf(file.path = pdf_dir, paste0("plot", i, ".pdf", sep = ""))

    

}
CHN <- file.path(pdf_dir, paste('Full_CHN.pdf', sep = ""))
staple_pdf(input_directory = pdf_dir, output_filepath = "CHN")

staple_pdf()

pdf_combine(pdf_dir, c("pdf_resultsCHN_flag_state_map.pdf","pdf_resultsCHN_subsidy_plot.pdf"), output = "joinedCHN.pdf")
...
staple_pdf(input_files = c("pdf_resultsCHN_flag_state_map.pdf", "pdf_resultsCHN_subsidy_plot.pdf"), output_filepath = file.path(pdf_dir, 'Full_pdf.pdf'))

##


output_file <- file.path(pdf_dir, paste('Full_CHN.pdf', sep = ""))
staple_pdf(input_directory = pdf_dir, output_filepath = output_file)

###
setwd("pdf_dir") ##
infiles <- Sys.glob("pdf_resultsCHN_*.pdf") ##
outfile <- "CHN.pdf" ##
system(paste("pdftk", paste(infiles, collapse = " "), "CHN", outfile))


##

mf <- file.path(dir, "merged.pdf")
merge_pdf(mf)
get_n_pages(mf)

p.1 = "pdf_resultsCHN_flag_state_map.pdf"
p.2 = "pdf_resultsCHN_subsidy_plot.pdf"

plots.list = list(p.1, p.2)

plots = do.call(marrangeGrob, c(plots.list, list(nrow = 1, ncol = 1)))

ggsave("FullCHN.pdf", plots)

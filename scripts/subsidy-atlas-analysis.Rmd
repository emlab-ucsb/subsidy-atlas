...
dw_effort_final_manual <- dw_effort_final %>%
...
if(rerun_gfw){
  ...
  if(bq_table_exists(dw_effort_final_table)){
      ...
      dw_effort_final_table %>%
      bq_table_create() %>%
      bq_table_upload(values = dw_effort_final_manual, fields = dw_effort_final_manual)
...
write_csv(dw_effort_final_manual, paste0(results_dir, "FINAL_dw_effort_by_eez_", analysis_year, ".csv"))
...
eez_mapping_wlines_africa <- eez_mapping %>%
  ...
  st_sfc(crs = 4326) %>%
  st_sf(geometry = .) %>% 
  bind_cols(eez_mapping %>% dplyr::filter(flag!= "UNK") %>% dplyr::filter(region == "Africa")) %>%
  select(everything(), geometry) %>% 
  st_segmentize(units::set_units(100,km)) %>% 
  mutate(geometry = (geometry + c(180,90)) %% c(360) - c(180,90)) %>% 
  st_wrap_dateline(options = c("WRAPDATELINE=YES",  "DATELINEOFFSET=180"), quiet = TRUE) %>% 
...
for(i in 54:length(eez_loop_iso3)){

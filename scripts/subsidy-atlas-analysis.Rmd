...
# Set directories for flag state results 
...
flag_state_tables_dir <- paste0(flag_state_results_dir, "tables/")
if (dir.exists(flag_state_tables_dir) == F) {
  dir.create(flag_state_tables_dir, recursive = T)
}

flag_state_country_dat_dir <- paste0(flag_state_results_dir, "country_dat/")
if (dir.exists(flag_state_country_dat_dir) == F) {
  dir.create(flag_state_country_dat_dir, recursive = T)
}

flag_state_figures_dir <- paste0(flag_state_results_dir, "figures/")
if (dir.exists(flag_state_figures_dir) == F) {
  dir.create(flag_state_figures_dir, recursive = T)
}

flag_state_pdf_dir <- paste0(flag_state_results_dir, "pdfs/")
if (dir.exists(flag_state_pdf_dir) == F) {
  dir.create(flag_state_pdf_dir, recursive = T)
}
...
eez_tables_dir <- paste0(eez_results_dir, "tables/")
if (dir.exists(eez_tables_dir) == F) {
  dir.create(eez_tables_dir, recursive = T)
}

eez_dat_dir <- paste0(eez_results_dir, "dat/")
if (dir.exists(eez_dat_dir) == F) {
  dir.create(eez_dat_dir, recursive = T)
}

eez_figures_dir <- paste0(eez_results_dir, "figures/")
if (dir.exists(eez_figures_dir) == F) {
  dir.create(eez_figures_dir, recursive = T)
}

eez_pdf_dir <- paste0(eez_results_dir, "pdfs/")
if (dir.exists(eez_pdf_dir) == F) {
  dir.create(eez_pdf_dir, recursive = T)
}
...
# Save top flag states by average ranking
write_csv(flag_state_ranking_top, paste0(flag_state_tables_dir, "top_", number_of_top_countries_flag, "_flag_states_avg_", analysis_year, ".csv"))

# Let's also make top lists for each category as well
# Vessels
write_csv(flag_state_ranking %>%
            top_n(-number_of_top_countries_flag, wt = vessel_rank), 
          paste0(flag_state_tables_dir, "top_", number_of_top_countries_flag, "_flag_states_vessels_", analysis_year, ".csv"))

# Total capacity
write_csv(flag_state_ranking %>%
            top_n(-number_of_top_countries_flag, wt = capacity_rank), 
          paste0(flag_state_tables_dir, "top_", number_of_top_countries_flag, "_flag_states_capacity_", analysis_year, ".csv"))

# Fishing hours
write_csv(flag_state_ranking %>%
            top_n(-number_of_top_countries_flag, wt = fishing_hours_rank), 
          paste0(flag_state_tables_dir, "top_", number_of_top_countries_flag, "_flag_states_fishing_hours_", analysis_year, ".csv"))

# Fishing KWh
write_csv(flag_state_ranking %>%
            top_n(-number_of_top_countries_flag, wt = fishing_KWh_rank), 
          paste0(flag_state_tables_dir, "top_", number_of_top_countries_flag, "_flag_states_fishing_KWh_", analysis_year, ".csv"))
...
st_write(flag_state_mapping_wlines, dsn = paste0(flag_state_tables_dir, "flag_state_mapping_with_lines.shp"),  delete_dsn=TRUE, driver = "ESRI Shapefile")
```
Do some setup for making our figures
...
If we're running the flag state analysis for a top number of countries... (only option currently)

```{r}
if(flag_state_purpose == "top_ranked"){
...
# If purpose of EEZ analysis is to do it for the top-ranked eezs... 
...
write_csv(EEZ_ranking_top, paste0(eez_tables_dir, "top_", number_of_top_countries_eez, "_EEZs_avg_", analysis_year, ".csv"))
...
write_csv(EEZ_ranking %>%
          ...
          paste0(eez_tables_dir, "top_", number_of_top_countries_eez, "_EEZs_vessels_", analysis_year, ".csv"))

# Total capacity
write_csv(EEZ_ranking %>%
            top_n(-number_of_top_countries_eez, wt = capacity_rank), 
          paste0(eez_tables_dir, "top_", number_of_top_countries_eez, "_EEZs_capacity_", analysis_year, ".csv"))

# Fishing hours
write_csv(EEZ_ranking %>%
            top_n(-number_of_top_countries_eez, wt = fishing_hours_rank), 
          paste0(eez_tables_dir, "top_", number_of_top_countries_eez, "_EEZs_fishing_hours_", analysis_year, ".csv"))

# Fishing KWh
write_csv(EEZ_ranking %>%
            top_n(-number_of_top_countries_eez, wt = fishing_KWh_rank), 
          paste0(eez_tables_dir, "top_", number_of_top_countries_eez, "_EEZs_fishing_KWh_", analysis_year, ".csv"))
...
if(eez_purpose == "top_ranked"){
  ...
  analysis_eez_names <- EEZ_ranking_top$eez_territory_iso3
  analysis_eez_names_long <- EEZ_ranking_top$eez_name
  analysis_eez_names_long[which(top_eezs == "8389")] <- "Falkland Islands Exclusive Economic Zone: UK / Argentina"

}else if(eez_purpose == "ACP"){
  
  ACP_codes <- read_csv(here::here("scripts", "ACP_eez_codes.csv")) %>% 
  filter(eez_id != "N/A")
  
  ACP_eez_filter <- EEZ_ranking %>%
    dplyr::filter(eez_code %in% ACP_codes$eez_id)
  
  analysis_eezs <- ACP_eez_filter$eez_code
  analysis_eez_names <- ACP_eez_filter$eez_territory_iso3
  analysis_eez_names_long <- ACP_eez_filter$eez_name
...
st_write(eez_mapping_wlines, dsn = paste0(eez_tables_dir, "eez_mapping_with_lines.shp"),  delete_dsn=TRUE, driver = "ESRI Shapefile")
...
for(i in 1:length(analysis_eezs)){

eez_dat <- eez_mapping_wlines %>%
  dplyr::filter(eez_code == analysis_eezs[i]) 

# Write EEZ data
write_csv(eez_dat, path = paste0(eez_dat_dir, analysis_eezs[i], "_", analysis_eez_names[i] , "_EEZ_dat.csv"))
...
eez_map <- ggplot()+
  ...
  geom_sf(data = EEZ_map %>% dplyr::filter(mrgid == analysis_eezs[i]), fill = "slateblue", alpha = 0.5, color = NA, size = 0.1) + # highlighted EEZs (slate blue)
...
#plot_map pngs
ggsave(eez_map, filename = paste0(eez_figures_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_EEZ_connectivity_map.png"), width = 10, units = "in")

# Combine map into a one page pdf
title3 <- textGrob(paste0("Global network of distant-water fishing\nEEZ: ", analysis_eez_names_long[i]), gp = gpar(fontsize=16, fontface="bold", lineheight=1, col = "white", fontfamily = "Helvetica"), hjust = 0.55, vjust = 1)

caption3 <- textGrob(paste0("This map shows the global origins of distant-water vessels fishing in the ", analysis_eez_names_long[i], ". Lines connect the EEZ (blue) \nwith all foreign countries (pink) from which vessels fishing in that EEZ in 2018 are flagged. Data were taken from Global Fishing Watch (2018)."), gp = gpar(fontsize = 10, col = "white", fontfamily = "Helvetica"), vjust = 0, hjust = 0.5)
...
# Save connectivity map
ggsave(out3, filename = paste0(eez_pdf_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_map_page.pdf"), width = 11, height = 8.5, units = "in")
...
# Pull data from GFW if necessary
 if(rerun_gfw == T){
  ...
  eez_info AS (
    ...
    HAVING
      year = ", analysis_year, 
      " AND eez_code = '", as.character(analysis_eezs[i]), "')
 ...
 SELECT
  ...
  eez_table <- bq_table(project = ucsb_project, table = paste0("fishing_effort_", analysis_eezs[i]), dataset = "subsidy_atlas") 

  if(bq_table_exists(eez_table) == FALSE){
    bq_table(project = ucsb_project, table = paste0("fishing_effort_", analysis_eezs[i]), dataset = "subsidy_atlas") %>%
      bq_table_create()
  }else{
    bq_table(project = ucsb_project, table = paste0("fishing_effort_", analysis_eezs[i]), dataset = "subsidy_atlas") %>%
      bq_table_delete()
  ...
  eez_effort <- bq_project_query(ucsb_project, sql3, destination_table = bq_table(project = ucsb_project, table = paste0("fishing_effort_", analysis_eezs[i]), dataset = "subsidy_atlas"), use_legacy_sql = FALSE, allowLargeResults = TRUE) %>%
    bq_table_download(max_results = Inf)

  write_csv(eez_effort, path = paste0(eez_dat_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_EEZ_effort_subs_by_flag.csv"))
  write_csv(eez_effort, path = paste0(eez_results_app_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_EEZ_effort_subs_by_flag.csv"))

  # Load local data instead of rerunning global fishing watch pull 
  } else{
  
  eez_effort <- read_csv(paste0(eez_dat_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_EEZ_effort_subs_by_flag.csv"), col_types = "iccnnnnn")
...
# Save connectivity map
ggsave(out4, filename = paste0(eez_pdf_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_effort_page.pdf"), width = 11, height = 8.5, units = "in")

# Save both plots into one pdf
  out_eez <- marrangeGrob(grobs = list(out3, out4), nrow = 1, ncol = 1, top = NULL)
  
  ggsave(out_eez, filename = paste0(eez_pdf_dir, analysis_eezs[i], "_", analysis_eez_names[i], "_combined.pdf"), width = 11, height = 8.5, units = "in")

}
```

# Make connectivity maps for all selected EEZs

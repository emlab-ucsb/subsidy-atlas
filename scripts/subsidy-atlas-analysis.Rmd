...
subsidy_table <- bq_table(project = ucsb_project, table = "subsidy_rates_flag", dataset = "subsidy_atlas")
  if(bq_table_exists(subsidy_table) == FALSE){
    bq_table(project = ucsb_project, table = "subsidy_rates_flag", dataset = "subsidy_atlas") %>%
      bq_table_create() %>%
      bq_table_upload(dw_subsidy_rates_flag %>% na.omit())
  }else{
    bq_table(project = ucsb_project, table = "subsidy_rates_flag", dataset = "subsidy_atlas") %>%
      bq_table_delete() %>%
      bq_table_upload(na.omit(dw_subsidy_rates_flag))
  }
}
...
#plot_map
ggsave(eez_map, filename = paste0(eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_map.png"), width = 10, units = "in")
...
bq_project_query(ucsb_project, sql3, destination_table = bq_table(project = ucsb_project, table = paste0("fishing_effort_", top_eezs[i]), dataset = "subsidy_atlas"), use_legacy_sql = FALSE, allowLargeResults = TRUE)

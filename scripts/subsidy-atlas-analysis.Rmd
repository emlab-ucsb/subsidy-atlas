...
In order to make our connection plots, we need to find the centroids of each EEZ and flag state before we go too much further. 
...
if((run_flag_state & flag_state_purpose == "top_ranked") | (run_eez & eez_purpose == "top_ranked")){
  ...
  eez_map <- st_crop(eez_map, c(xmin=-180, xmax=180, ymin=-90, ymax=90))
  ...
  # ggplot(eez_map) +
  #   geom_sf() +
  #   geom_point(data = eez_map_centroid_coordinates, aes(x = eez_lon, y = eez_lat))
  
  # Manually correct certain EEZ centroids 
  # NEED TO DO THIS
  ...
  land_eez_map <- st_crop(land_eez_map, c(xmin=-180, xmax=180, ymin=-90, ymax=90))
...
# For the ACP analysis, things get a little more complicated as we want to use three different world views for the three different regions
...
# Get centroids of each eez for africa
...
acp_map <- read_sf(dsn = here::here("data", "shapefiles", "World_EEZ_v10_custom_ACP"), layer = "World_EEZ_v10_custom_ACP") %>% 
  st_transform(crs = 4326)

acp_map_centroids <- acp_map %>% 
  #dplyr::filter(is.na(zone)) %>%
  st_centroid() %>%
  dplyr::select(eez_code = mrgid, eez_name = geoname, geometry)

centroid_coordinates_EEZ_acp <- tibble(eez_code = acp_map_centroids$eez_code,
                                   eez_name = acp_map_centroids$eez_name) %>% 
  bind_cols(do.call(rbind, st_geometry(acp_map_centroids)) %>% 
    as_tibble() %>% setNames(c("eez_lon","eez_lat"))) %>%
  arrange(eez_code)

centroid_coordinates_EEZ_acp_edited <- centroid_coordinates_EEZ_acp %>% 
...
if(eez_purpose == "top_ranked"){
  ...
  ACP_codes <- read_csv(here::here("SubsidyAtlasACP", "data", "ACP_eez_codes.csv")) %>% 
    dplyr::select(territory, territory_iso3, mrgid) %>%

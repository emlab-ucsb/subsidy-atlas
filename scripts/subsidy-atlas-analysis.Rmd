...
eez_effort <- read_csv(paste0(eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_effort_subs_by_flag.csv"))
...
### Make Plots 
  ...
  # 95% quantiles
  intensity_quantile <- quantile(eez_effort_aggregated$subsidy_intensity, probs = c(0.95, 0.99), na.rm = T)
  
  # Map of subsidy intensity
  eez_subsidy_plot <- ggplot(eez_effort_aggregated, aes(x = lon_cen, y = lat_cen, fill = subsidy_intensity))+
    geom_raster()+
    eezmaptheme +
    scale_fill_viridis_c(na.value = NA, name = "Subsidy intensity \n(2018 $USD/KWh)", limits=c(0, intensity_quantile[1]), oob=scales::squish)+
    #coord_fixed(ratio = 1)+
    labs(x = "Longitude", y = "Latitude")
  
  eez_effort_plot <- ggplot(eez_effort_aggregated, aes(x = lon_cen, y = lat_cen, fill = log(fishing_KWh)))+
    geom_raster()+
    eezmaptheme +
    scale_fill_viridis_c(na.value = NA, option = "A", name = "Fishing effort (log(KWh))")+
    #coord_fixed(ratio = 1)+
    labs(x = "Longitude", y = "Latitude")
 
  # Combined plot  
  eez_combined_plot <- ggarrange(eez_effort_plot, eez_subsidy_plot + rremove("y.title"), ncol = 2, nrow = 1) 
  
  ggsave(filename = paste0(eez_dir, top_eezs[i], "_", top_eez_names[i], "_EEZ_heat_map.png"), eez_combined_plot, width = 10, height = 5, units = "in")

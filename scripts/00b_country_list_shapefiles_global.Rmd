...
This script creates the global country list used in the Distant Water Fishing Atlas, as well as all necessary GeoDatabase files used in the tool. This relies upon the following data sources: 
- Flanders Marine Institute (2019). Maritime Boundaries Geodatabase: Maritime Boundaries and Exclusive Economic Zones (200NM), version 11. Available online at https://www.marineregions.org/. https://doi.org/10.14284/386
- World shapefile from the 
...
eez_attrib_all <- eez_attrib %>%
  ...
  dplyr::select(mrgid, pol_type, geoname, iso_ter_all, iso_sov_all, x_cen = x_1, y_cen = y_1)
...
others_sf <- eez_sf_out %>%
  ...
  dplyr::select(region, name_ter, iso_ter, name_sov, iso_sov, pol_type, geoname_new, x_cen, y_cen, geom)
...
Since we were a bit more thoughtful about the administering vs. sovereign states for our land polygons, let's use this to create a new lookup table of administering states and their corresponding sovereigns - we can also apply this to our EEZ file later to create one that encompasses both

```{r}
land_admin_sov_lookup <- land_out %>%
  arrange(sovereign, admin, type) %>%
  st_drop_geometry()

eez_ter_sov_lookup <- eez_sf_out %>%
  st_drop_geometry() %>%
  group_by(iso_ter, name_ter, iso_sov, name_sov) %>%
  summarize(all_eez_ids = paste0(mrgid, collapse = ","))

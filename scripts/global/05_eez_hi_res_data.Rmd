...
app_dir <- here::here("app-global/data/eez-effort-subs/")
...
This script pulls high resolution distant water fishing effort data for each EEZ. 
...
vessel_list_expanded <- vessel_list %>%
  rowwise() %>%
...
```{r}
flags_by_eez_id <- vessel_list_expanded %>%
...
for(i in 1:length(eez_ter_to_pull)){
 ...
 SELECT
  ...
  write_csv(effort_eez_save, paste0(app_dir, eez_ter_to_pull[i], "_eez_effort_subs_by_flag.csv"))

...
title: "Distant Water Fishing Atlas - 02: Global Vessel List"
...
The process of pulling this data as detailed below is identical to the process used to create the dataset underlying the SubsidyExplorer toolkit. 
...
if(bq_table_exists(fv_table)){
  ...
  good_fishing_vessels <- bq_project_query(bq_project, 
                                           sql,
...
We don't store the output from the previous query locally as it is too large (and we have no reason to keep this locally), but we'll download it into the environment so it can be visualized here when rendered to HTML. 

```{r}
# Trying to display the entire data frame freezes the computer - Output table has 301,963 rows and 4 columns.
good_vessels_to_display <- head(good_fishing_vessels, n = 1000)
datatable(good_vessels_to_display, options = list(pageLength = 10))
```
...
- The fishing activity must have occurred during a segment with more than 10 positions that was not concurrently overlapping with another, longer, segment for the same MMSI.  
...
- eez_territory1_iso3 (chr) : Three letter code correspondig to the administering territory of the EEZ
- eez_territory2_iso3 (chr) : Three letter code corresponding to the second administering territory of the EEZ (disputed and/or joint regime areas only)
- eez_territory3_iso3 (chr) : Three letter code corresponding to the third administering territory of the EEZ (disputed and/or joint regime areas only)
- eez_sovereign1_iso3 (chr) : Three letter code corresponding to the sovereign state of the EEZ
- eez_sovereign2_iso3 (chr) : Three letter code corresponding to the second sovereign state of the EEZ (disputed and/or joint regime areas only)
- eez_sovereign3_iso3 (chr) : Three letter code corresponding to the third sovereign state of the EEZ (disputed and/or joint regime areas only)
...
- engine_power_kw (dbl): Best engine power in kilo-watt hours. 
...
WITH
  ...
  SELECT
    *
  FROM
    `emlab-gcp.distant_water_fishing_atlas.all_good_fishing_vessels`),
...
This file is saved locally, but is also displayed below when rendered to HTML. 

```{r}
# Trying to display the entire data frame freezes the computer - Output table has 143,951 rows and 21 columns. 
effort_good_vessels_to_display <- head(effort_good_fishing_vessels_eez, n = 1000)
datatable(effort_good_vessels_to_display, options = list(pageLength = 10))
```
# Correct outdated flag codes and Identify EU and USA dependent vessels

Before allocating catches and subsidies across vessels, we need to do some manual corrections for incorrect flag codes and we need to identify vessels flagged to states that are overseas dependencies of the EU or USA. We assume that these vessels receive subsidies from their sovereign states. 
...
This process is fairly straightforward as we only have data by flag. Let's first assign sovereigns to all of our flags. 
...
# And manually correct two so their sovereign states match those recognized by the WTO - Clipperton Atoll and Western Sahara

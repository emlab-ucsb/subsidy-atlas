...
results_dir <- here::here("results/00a-eez-polygon-wrangling/")
...
write_csv(eez_attrib, paste0(results_dir, "eez_attributes.csv"))
...
# Manually assign regions for those that are missing
...
```{r}
  ...
  east_asia_pacific_eez_centroids <- east_asia_pacific_eez_sf %>%
    st_centroid() %>%
    dplyr::select(mrgid, eez_ter_iso3 = iso_ter, eez_ter_name = name_ter, eez_sov_iso3 = iso_sov, eez_sov_name = name_sov, region) %>%
  ...
  # Save
  write_csv(eez_centroid_coordinates, paste0(results_dir, "eez_centroid_coordinates.csv"))
...
eez_sf_out <- eez_sf_out %>%
  ...
  left_join(eez_centroid_coordinates, by = c("mrgid", "pol_type", "geoname", "eez_ter_iso3", "eez_ter_name", "eez_sov_iso3", "eez_sov_name",  "region"))
```

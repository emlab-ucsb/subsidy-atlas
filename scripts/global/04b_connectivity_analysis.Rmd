...
# Path to eez file
eez_centroid_path <- here::here("results", "00a-eez-polygon-wrangling", "eez_centroid_coordinates.csv")

# Path to land file
land_centroid_path <- here::here("results", "00b-land-polygon-wrangling", "land_centroid_coordinates.csv")
...
Since our vessel list of distant water fishing (and subsidies) is currently at the individual vessel level, we first want to aggregate this by flag state/coastal state. 
...
vessel_list_expanded <- vessel_list_raw %>%
  rowwise() %>%
...
flag_eez_summary <- vessel_list_expanded %>%
...
# Create an entry for the EU as a whole just because
...
eu_summary <- vessel_list_expanded %>%
...
Now let's load in our land and EEZ centroids so we can start matching them up. 

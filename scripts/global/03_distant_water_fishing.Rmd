...
vessel_list_update <- vessel_list_raw %>%
  ...
  mutate(flag_sovereign_iso3 = case_when(flag_iso3 %in% c("HKG", "TWN") ~ "CHN",
                                         TRUE ~ flag_sovereign_iso3)) # fix Hong Kong and Taiwan
```
...
- All fishing effort occuring in an EEZ when the sovereign flag state of the vessel is the same as the administering state of the EEZ in which it is fishing. 
...
# Remove cases where flag state of vessel (or sovereign flag state of vessel) matches the administering territory (or territories in the cases of joint regime/disputed areas)
distant_water_fishing <- vessel_list_dw %>%
   mutate(ter1_match = case_when(!is.na(iso_ter1) & !is.na(flag_iso3) & flag_iso3 == iso_ter1 ~ T,
         ...
         ter3_match = case_when(!is.na(iso_ter3) & !is.na(flag_iso3) & flag_iso3 == iso_ter3 ~ T,
                                      TRUE ~ F),
         ter1_sov_match = case_when(!is.na(iso_ter1) & !is.na(flag_sovereign_iso3) & flag_sovereign_iso3 == iso_ter1 ~ T,
                                 TRUE ~ F),
         ter2_sov_match = case_when(!is.na(iso_ter2) & !is.na(flag_sovereign_iso3) & flag_sovereign_iso3 == iso_ter2 ~ T,
                                      TRUE ~ F),
         ter3_sov_match = case_when(!is.na(iso_ter3) & !is.na(flag_sovereign_iso3) & flag_sovereign_iso3 == iso_ter3 ~ T,
                                      TRUE ~ F)
         ) %>%
  dplyr::filter(!ter1_match & !ter2_match & !ter3_match & !ter1_sov_match & !ter2_sov_match & !ter3_sov_match)
...
2. The sovereign of the vessel's flag state is a member state of the EU, or is Norway, Svalbard and Jan Mayen, or Iceland and the EEZ in which it is fishing is administered by a member state of the EU, Norway, Svalbard and Jan Mayen, or Iceland (e.g. a French Polynesian-flagged vessel fishing in the EEZ of France).

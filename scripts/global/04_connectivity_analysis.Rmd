...
# Path to updated EEZ characteristics
eez_attrib_path <- here::here("results", "global", "00a-eez-polygon-wrangling", "eez_attributes_expanded.csv")
...
eez_attributes <- read_csv(eez_attrib_path)

# Extract eez_ids and cooresponding ter and sov iso3 codes
eez_attrib_expand <- eez_attributes %>%
  distinct(mrgid, iso_sov, iso_ter) %>%
  rename(eez_id = mrgid)
```

```{r}
# Let's duplicate entries for EEZs with multiple administering territories (old method)
...
# Let's duplicate entries for EEZs with multiple administering territories (new method)
vessel_list_dw_expanded <- vessel_list_dw_raw %>%
   dplyr::select(-eez_territory1_iso3, -eez_territory2_iso3, -eez_territory3_iso3, -eez_sovereign1_iso3, -eez_sovereign2_iso3, -eez_sovereign3_iso3) %>%
  mutate(eez_id = case_when(eez_id == 8344 ~ as.integer(62589), # updating code for Chagos
                            TRUE ~ eez_id)) %>%
  left_join(eez_attrib_expand, by = "eez_id") %>%
  rename(eez_ter_iso3 = iso_ter,
         eez_sov_iso3 = iso_sov) %>%
  dplyr::filter(!is.na(eez_ter_iso3)) # removes 3 entries
...
# Now calculate summaries for each coastal territory and expand so there are duplicate entries for multiple non-continuous EEZs
flag_eez_summary <- vessel_list_dw_expanded %>%
  rename(flag_is_EU = is_EU) %>%
  mutate(eez_is_EU = case_when(eez_ter_iso3 %in% eu_countries ~ T,
                               TRUE ~ F),
         flag_is_EU = case_when(!is.na(flag_is_EU) ~ flag_is_EU,
                                TRUE ~ F)) %>%
...
eez_summary <- vessel_list_dw_expanded %>%
  mutate(eez_is_EU = case_when(eez_ter_iso3 %in% eu_countries ~ T,
                               TRUE ~ F)) %>%
  group_by(eez_ter_iso3, eez_is_EU) %>%
  summarize(n_vessels = n_distinct(ssvid),
            mean_length = mean(length_m, na.rm = T),
            mean_tonnage = mean(tonnage_gt, na.rm = T),
            tot_tonnage = sum(tonnage_gt, na.rm = T),
...
write_csv(eez_summary, paste0(app_dir, "dw_activity_summary_by_eez.csv"))
```
...
flag_state_summary <- vessel_list_dw_expanded %>%
  rename(flag_is_EU = is_EU) %>%
  mutate(flag_is_EU = case_when(!is.na(flag_is_EU) ~ flag_is_EU,
                                TRUE ~ F)) %>%
  group_by(flag_iso3, flag_is_EU) %>%
  summarize(n_vessels = n_distinct(ssvid),
            mean_length = mean(length_m, na.rm = T),
            mean_tonnage = mean(tonnage_gt, na.rm = T),
            tot_tonnage = sum(tonnage_gt, na.rm = T),
...
write_csv(flag_state_summary, paste0(app_dir, "dw_activity_summary_by_flag_state.csv"))
```

# Flag state / FAO Region Summaries (Distant Water Fishing)

Let's now do the same thing for our high seas data 
...
```{r}
flag_fao_summary <- vessel_list_hs_raw %>%
  rename(flag_is_EU = is_EU) %>%
  mutate(flag_is_EU = case_when(!is.na(flag_is_EU) ~ flag_is_EU,
                                TRUE ~ F)) %>%
...
Now let's create a region summary version...
...
write_csv(fao_summary, paste0(app_dir, "hs_activity_summary_by_area.csv"))
```

And finally the same thing for flag states fishing on the high seas...
...
write_csv(hs_flag_state_summary, paste0(app_dir, "hs_activity_summary_by_flag_state.csv"))
```

...
ACP_codes <- read_csv("SubsidyAtlasACP/data/ACP_eez_codes.csv", locale = locale(encoding = "UTF-8")) #%>% 
  #colnames(ACP_codes) <- character
...
eez_effort_ZAF <- read_csv("SubsidyAtlasACP/data/eez_results/ACP/8396_ZAF_EEZ_effort_subs_by_flag.csv") %>% 
      as.data.frame()
    ...
    ggplot()+
      geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.5)+ # world EEZs (transparent, light grey border lines)
      geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.5)+ # world countries (dark grey, white border lines)
      geom_tile(data = eez_effort_aggregated_VUT, aes(x = lon_cen, y = lat_cen, fill = fishing_KWh))+
      #eezmaptheme +
      scale_fill_viridis_c(na.value = NA, option = "A", name = "Fishing effort \n(KWh)", trans = log10_trans(), labels = comma)+
      labs(x = "", y = "")+
      coord_sf(xlim = x_lim, ylim = y_lim) +
...
eez_effort_ZAF_BLZ <- eez_effort_ZAF %>% 
  filter(flag == "BLZ")
...
#Generate plot
     ...
     ### Make Plots 
     # Get quantiles of data for scale limits
     eez_effort_aggregated_ZAF_BLZ <- eez_effort_ZAF_BLZ %>%
       group_by(year, eez_code, lon_cen, lat_cen) %>%
       summarize(fishing_hours = sum(fishing_hours, na.rm = T),
                 fishing_KWh = sum(fishing_KWh, na.rm = T),
                 subs = sum(subs, na.rm = T)) %>%
     ...
     # # # Quantiles
     intensity_quantile <- quantile(eez_effort_aggregated_ZAF_BLZ$subs, probs = c(0.01, 0.05, 0.95, 0.99), na.rm = T)
     scale_labels <- round(seq(round(intensity_quantile[1], 1), round(intensity_quantile[4], 1), length.out = 5), 1)
     
    # # # Set map limits for filtering global country map and eez map appropriately
     x_lim <- c(min(eez_effort_aggregated_ZAF_BLZ$lon_cen) - 0.5, max(eez_effort_aggregated_ZAF_BLZ$lon_cen) + 0.5)
     y_lim <- c(min(eez_effort_aggregated_ZAF_BLZ$lat_cen) - 0.5, max(eez_effort_aggregated_ZAF_BLZ$lat_cen) + 0.5)
     
    # Map of subsidy intensity
    ggplot()+
      geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.5)+ # world EEZs (transparent, light grey border lines)
      geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.5)+ # world countries (dark grey, white border lines)
      geom_tile(data = eez_effort_aggregated_ZAF_BLZ, aes(x = lon_cen, y = lat_cen, fill = subs))+
      eezmaptheme +
      scale_fill_viridis_c(na.value = NA, name = "Subsidy intensity \n(2018 $USD/KWh)", 
                           limits=c(round(intensity_quantile[1],1)-round(intensity_quantile[1],1)*0.01, round(intensity_quantile[4], 1) + round(intensity_quantile[4], 1)*0.01), 
                           labels=scale_labels,
  ...
  # #Gemerate effort plot ZAF_BLZ
    ...
    ggplot()+
      geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.5)+ # world EEZs (transparent, light grey border lines)
      geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.5)+ # world countries (dark grey, white border lines)
      geom_tile(data = eez_effort_aggregated_ZAF_BLZ, aes(x = lon_cen, y = lat_cen, fill = fishing_KWh))+
      #eezmaptheme +
      scale_fill_viridis_c(na.value = NA, option = "A", name = "Fishing effort \n(KWh)", trans = log10_trans(), labels = comma)+
      labs(x = "", y = "")+
      coord_sf(xlim = x_lim, ylim = y_lim) +
...
eez_effort_ZAF_JPN <- eez_effort_ZAF %>% 
  filter(flag == "JPN")


#Generate subsidy plot
    
     ### Make Plots 
     # Get quantiles of data for scale limits
     eez_effort_aggregated_JPN <- eez_effort_ZAF_JPN %>%
       group_by(year, eez_code, lon_cen, lat_cen) %>%
       summarize(fishing_hours = sum(fishing_hours, na.rm = T),
                 fishing_KWh = sum(fishing_KWh, na.rm = T),
                 subs = sum(subs, na.rm = T)) %>%
     ...
     # # # Quantiles
     intensity_quantile <- quantile(eez_effort_aggregated_JPN$subs, probs = c(0.01, 0.05, 0.95, 0.99), na.rm = T)
     scale_labels <- round(seq(round(intensity_quantile[1], 1), round(intensity_quantile[4], 1), length.out = 5), 1)
     
    # # # Set map limits for filtering global country map and eez map appropriately
     x_lim <- c(min(eez_effort_aggregated_JPN$lon_cen) - 0.5, max(eez_effort_aggregated_JPN$lon_cen) + 0.5)
     y_lim <- c(min(eez_effort_aggregated_JPN$lat_cen) - 0.5, max(eez_effort_aggregated_JPN$lat_cen) + 0.5)
     
    # Map of subsidy intensity
    ggplot()+
      geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.5)+ # world EEZs (transparent, light grey border lines)
      geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.5)+ # world countries (dark grey, white border lines)
      geom_tile(data = eez_effort_aggregated_JPN, aes(x = lon_cen, y = lat_cen, fill = subs))+
      eezmaptheme +
      scale_fill_viridis_c(na.value = NA, name = "Subsidy value \n(2018 $USD)", 
                           limits=c(round(intensity_quantile[1],1)-round(intensity_quantile[1],1)*0.01, round(intensity_quantile[4], 1) + round(intensity_quantile[4], 1)*0.01), 
                           labels=scale_labels,
                           breaks=scale_labels,
                           oob=scales::squish)+
    ...
    ggplot()+
      geom_sf(data = EEZ_map %>% dplyr::filter(is.na(zone)), fill = NA, color = "grey60", size = 0.5)+ # world EEZs (transparent, light grey border lines)
      geom_sf(data = country_map, fill = "grey2", color = "grey40", size = 0.5)+ # world countries (dark grey, white border lines)
      geom_tile(data = eez_effort_aggregated_JPN, aes(x = lon_cen, y = lat_cen, fill = fishing_KWh))+
      #eezmaptheme +
      scale_fill_viridis_c(na.value = NA, option = "A", name = "Fishing effort \n(KWh)", trans = log10_trans(), labels = comma)+
      labs(x = "", y = "")+
      coord_sf(xlim = x_lim, ylim = y_lim) +
...
connectivity_data_for_selected_eez_MRT <- connectivity_data %>% # load this in up above
      dplyr::filter(eez_cod == "8614") %>% #input$africa_eez_select) %>% 
      arrange(flag)

#  Hover Text
    flag_state_summary_ZAF <- connectivity_data_for_selected_eez_ZAF %>%
      mutate(flag = ifelse(is.na(flag), "UNK", flag)) %>% 
      mutate(name = countrycode(flag, "iso3c", "country.name"))
    
  dw_effort_MUS <- dw_effort_final %>% 
    dplyr::filter(eez_code == "8343")
  
  dw_effort_GNB <- dw_effort_final %>% 
    dplyr::filter(eez_code == "8471")
  
  connectivity_data_for_selected_eez_MUS <- connectivity_data %>% # load this in up above
      dplyr::filter(eez_cod == "8343") %>% #input$africa_eez_select) %>% 
      arrange(flag)
  
  connectivity_data_for_selected_eez_GNB <- connectivity_data %>% # load this in up above
      dplyr::filter(eez_cod == "8471") %>% #input$africa_eez_select) %>% 
      arrange(flag)

```

```{r}
#Check to see if any gfw data in EEZ

dw_effort_test <- dw_effort_final %>% 
  dplyr::filter(eez_territory_iso3 == "MRT")

connectivity_data_test <- connectivity_data %>% 
  dplyr::filter(ez_tr_3 == "MRT")

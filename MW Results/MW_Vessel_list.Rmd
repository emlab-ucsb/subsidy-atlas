...
EEZ_tidy <- EEZ %>%
  mutate(flag = strsplit(d, ", ")) %>%
        unnest(flag) %>%
        dplyr::select(eez_iso3, number_countries_in_EEZ, flag) %>% 
  #rename(eez_fished = eez_iso3) %>% 
  select(eez_iso3, flag, number_countries_in_EEZ) %>% 
  arrange(eez_iso3)
...
country_map <- read_sf(dsn = here::here("data", "shapefiles", "world_happy_180"), layer = "world_happy_180") %>%   st_transform(crs = 4326)
...
```{r Combining final EEZ tables}


EEZ_final_results_table <- EEZ_tidy %>% 
  right_join(EEZ_results, by = c("eez_iso3", "flag")) %>% 
  select(flag, eez_iso3, vessels_per_eez, effort_per_eez, kwhr) %>% 
  right_join(centroid_coordinates_country, by = c("flag")) %>% 
  rename(startlon = lon, startlat = lat) %>% 
  right_join(centroid_coordinates_EEZ, by = c("eez_iso3")) %>%
  rename(endlon = lon, endlat = lat) %>%
  arrange(flag) %>% 
  drop_na()

EEZ_final_results <- EEZ_final_results_table %>% 
  ...
  st_sfc(crs = 4326) %>%
  st_sf(geometry = .) %>% 
  bind_cols(EEZ_final_results_table) %>%
...
write_csv(EEZ_final_results, here::here("MW Results","Final", "EEZ_final_results.csv"))

ggsave(filename = "EEZ_final_results.png")
...
```{r Final flag results: #vessels, kwrh, average length, and average tonnage per country}
